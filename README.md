...
- 首次启动会自动初始化默认账号 **`tiantian` / `tiantian`**

### 🔑 API Key 持久化

每个账号配置一次 MiMo API Key 后**自动绑定到当前账号**，后续登录无需重复输入，随时可在「高级设置」修改。
...
2. 使用默认账号 **`tiantian` / `tiantian`** 登录，或点「注册」创建新账号
3. 在右侧「高级设置」粘贴你的 MiMo API Key 并点保存
4. 选择模型 → 输入文本 → 点「生成语音」

> 💡 默认音色文件不随仓库分发。如需使用「音色克隆」的默认音色，请把任意 WAV 命名为 `诺艾尔.wav` 放在项目根目录；或直接在 UI 上传一份音频样本。
